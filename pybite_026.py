...
# A dictionary comprehension is like a list comprehension, 
#     but it constructs a dict instead of a list. 
...
#     Write a dictionary comprehension that filters out the items in the set 
...
def filter_bites(bites=bites, bites_done=exclude_bites):
    ...
    # return {k:filter(lambda x: x in exclude_bites, exclude_bites) for k,v in bites.items()}
    # return {k:filter(lambda x: x in exclude_bites, bites.keys()) for k,v in bites.items()}
